...
public partial class frmSAPPayrollEntriesUploader : Form
{
    ...
    public frmSAPPayrollEntriesUploader()
    {
        InitializeComponent();
    }

    private void frmSAPPayrollEntriesUploader_Load(object sender, EventArgs e)
    {
...
SELECT ZZ.EMPLOYEE, ZZ.EMPLOYEENAME, ZZ.ACCOUNT AS [JEACCT], ZZ.ACCOUNT, ZZ.ACCOUNTNAME, ZZ.SUDENTRY, 0.00 AS DEBIT, SUM(ZZ.Amount) AS CREDIT FROM (
...
SELECT '' AS EMPLOYEE, '' AS EMPLOYEENAME, '" + _SAPAcctCode + @"' AS [JEACCT], ZZ.ACCOUNT, ZZ.ACCOUNTNAME, ZZ.SUDENTRY, SUM(ZZ.Amount) AS DEBIT, 0.00 AS CREDIT FROM (
...
, D.BCode
  ...
  ORDER BY XXX.ACCOUNT, XXX.ACCOUNTNAME
